...
export const VirtualNeobanksSection: React.FC<VirtualNeobanksSectionProps> = ({
  ...
  const [selectedId, setSelectedId] = useState<string>(virtualRates[0]?.id);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
          {virtualRates.map((item) => {
            const asset = getCurrencyAsset(item.id.toUpperCase().includes('USDT') ? 'USDT' : item.currency === 'EUR' ? 'EUR' : 'USD');
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="text-right">
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{item.name.split(' ')[0]}</p>
           ...
           <div>
              <div className="flex items-center gap-4 mb-8">
                 <div className={`w-14 h-14 rounded-[1.25rem] flex items-center justify-center text-3xl shadow-inner ${isDark ? 'bg-slate-950' : 'bg-slate-100'}`}>
                    <span className="flag-emoji">{getCurrencyAsset(selectedItem.id.toUpperCase().includes('USDT') ? 'USDT' : selectedItem.currency === 'EUR' ? 'EUR' : 'USD').flag}</span>
                 </div>
                 <div>
